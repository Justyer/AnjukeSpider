...
class CommunitySpider(CrawlSpider):
    ...
    def start_requests(self):
        # id_and_route = Postgresql().query_by_sql('select di.id,di.route,ci.url from ajk_city ci,ajk_district di where di.city_id=ci.id')
        ...
        #     )
        id_and_route = Postgresql().query_by_sql('select id,route from ajk_district di where city_id=2')
