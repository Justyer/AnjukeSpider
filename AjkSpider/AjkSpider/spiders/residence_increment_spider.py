...
from AjkSpider.Exception.tryex import *

class ResidenceIrtSpider(CrawlSpider):
    name = 'ajk_get_residence_irt'
    start_urls = []
    custom_settings = {
        # 'JOBDIR': 'crawls/ajk_get_deal-1000-5000',
        # 'LOG_FILE': 'logs/ajk_esf_transaction.log',
    ...
    def __init__(self):
        ...
        d_c_q = Mysql().query_by_sql('''
            ...
            from t_web_ajk_district d,t_web_ajk_community c
    ...
    def start_requests(self):
        ...
        for url in url_set:
            rlt = Mysql().query_by_sql("select count(*) from t_web_ajk_residence where url='%s'" % url)
            if rlt == []:
                yield Request(
                    url,
