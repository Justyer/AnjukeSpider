...
class EsfIrtSpider(CrawlSpider):
    ...
    custom_settings = {
        ...
        'JOBDIR': '/usr/local/crawler/dxc/common/ajk/crawls/ajk_esf_irt_%s' % datetime.date.today(),
        ...
        'DOWNLOADER_MIDDLEWARES':{
            ...
            # 'AjkSpider.middlewares.ProxyxxxMiddleware': 302,
        },
        'ITEM_PIPELINES':{
    ...
    def get_esf_url(self,response):
        ...
        if fabu_time_tian != []:
            return
        if int(fabu_time_tian[0]) == 0 or int(fabu_time_tian[0]) > 1:
            return
    ...
    def get_esf_info(self, response):
        ...
        item['bsn_dt']            = datetime.date.today()
