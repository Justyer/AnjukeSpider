...
class EsfIrtSpider(CrawlSpider):
    ...
    custom_settings = {
        ...
        'DOWNLOADER_MIDDLEWARES':{
            ...
            'AjkSpider.middlewares.CaptchaMiddleware': 203,
        },
        'ITEM_PIPELINES':{
    ...
    def get_esf_url(self,response):
        print 'start----------'
        if response.url[0:38] == 'https://www.anjuke.com/captcha-verify/':
            print 'form request |||||||||||------------++++++++++++'
            yield FormRequest.from_response(
                ...
                formdata={
                    'code': 'fdsb3',
                    'token': response.request.meta['token1'],
                    'token2': response.request.meta['token2'],
                    'submit': u'提交'
                },
            ...
            esf_url = [Selector(response).xpath('//*[@id="houselist-mod-new"]/li/div[2]/div[1]/a/@href').extract_first()]
            for url in esf_url:
